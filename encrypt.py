...
def encrypt_DSA(text: str, key_size: int = 2048) -> str:
    ...
    try:
        ...
        return f"Error in DSA encryption: {str(e)}"
...
def encrypt_ElGamal(text: str, key_size: int = 1024) -> str:
    ...
    try:
        ...
        return f"Error in El Gamal encryption: {str(e)}"
...
def main(json_str: str) -> str:
    ...
    if method == 'caesar':
        res['Encrypted text'] = encrypt_caesar(text, int(params['a']))
    elif method == 'affine':
        res['Encrypted text'] = encrypt_affine(text, int(params['a']), int(params['b']))
    elif method == 'multiplicative':
        res['Encrypted text'] = encrypt_multiplicative(text, int(params['a']))
    elif method == 'rsa':
        res['Encrypted text'], res['Public key'], res['Private key'], res['n'] = encrypt_RSA(text, int(params['primeP']), int(params['primeQ']))
    elif method == 'permutation':
        res['Encrypted text'] = encrypt_permutation(text, params['m'], params['pi'])
    elif method == 'hill':
        res['Encrypted text'] = encrypt_hill(text, params['matrix'])
    elif method == 'vigenere':
        res['Encrypted text'] = encrypt_vigenere(text, params['key'])
    elif method == 'aes':
        res['Encrypted text'], res['Key'], res['IV'] = encrypt_AES(text, params['key'], params['mode'])
    elif method == 'des':
        res['Encrypted text'], res['Key'], res['IV'] = encrypt_DES(text, params['key'], params['mode'])
    elif method == 'dsa':
        res['Signature'], res['Public key'], res['Private key'] = encrypt_DSA(text, int(params['key_size']))
    elif method == 'elgamal':
